...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['txtAuthorName'])) {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("si", $tenTacGia, $id);

        if ($stmt->execute()) {
            echo "<script>alert('Cập nhật tác giả thành công!'); window.location.href='author.php';</script>";
        } else {
...
<body>
    ...
    <main class="container mt-5 mb-5">
        <div class="row">
            <div class="col-sm">
                <h3 class="text-center text-uppercase fw-bold">Chỉnh sửa thông tin tác giả</h3>
                <form action="edit_author.php?id=<?php echo $id;?>" method="post">
                    <div class="input-group mt-3 mb-3">
                        <span class="input-group-text" id="lblAuthorName">Tên tác giả</span>
                        <input type="text" class="form-control" name="txtAuthorName"
                            value="<?php echo $row['ten_tgia'];?>" required>
                    </div>
